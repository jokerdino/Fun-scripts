...
def get_api_data(query):
    ...
    try:
        ...
        print "Unknown or unsupported site. The following sites are supported:"
...
stats = (username,
    ...
    user_data['question_count'], #accepted, accept_rate, //accept_rate doesn't look for closed questions, wiki, no answers and other problems
...
print u"""
  Statistics for \33[1m%s%s\33[m (%d):
   ...
   - Commented %d times (received %d 'great comment' votes)
   - The average comment received %.2f votes (min: %d, max: %d)

   - Voted on %d posts (%d upvotes, %d downvotes)
   - Edited %d entries (%d without re-tagging)

   - Total votes received: %+d
   - Received %d up-votes (%d on questions, %d on answers)
   - Received %d down-votes (%d on questions, %d on answers)
...
repfromq = upvotes_q * 5 - downvotes_q * 2 + accepted * 2
repfroma = upvotes_a * 10 - downvotes_a * 2 + accepted_answers * 15
 
print "   - Reputation from questions:",repfromq #doesn't exclude self-accepted answers +2 and community-wiki
print "   - Reputation from answers:",repfroma #doesn't excluce wiki answers


upvotesreceived = user_data['up_vote_count']
upvotesgiven = upvotes_q + upvotes_a


votedensity = round(float (upvotesreceived) / float (upvotesgiven),2)
if (votedensity > 1 ):
	print "   - Upvotes received : Upvotes given = 1 :",votedensity
else:
	print "   - Upvotes blackhole alert"
current_rep = user_data['reputation']

days = (datetime.now() - datetime.fromtimestamp(user_data['creation_date'])).days

avg_rep = current_rep/days
print "   - Average rep per day:",avg_rep
if (username=='jokerdino'):
	target_rep = 15000
	diff_rep = target_rep - current_rep                         
	if (avg_rep > 0):
		print "   - Estimated number of days to reach your target reputation:",diff_rep/avg_rep
	else:
		print "   - Average rep per day is almost equal to zero."
